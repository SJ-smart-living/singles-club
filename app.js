...
privacyChip:"资料默认不公开",eyebrow:"LOCAL SOCIAL EXPERIENCES",
...
privacyChip:"Private by default",eyebrow:"LOCAL SOCIAL EXPERIENCES",
...
async function request(path,opt={}){const r=await fetch(API+path,opt);let j=null;try{j=await r.json()}catch{}if(!r.ok)throw new Error(j?.error||`Request failed (${r.status})`);return j}
...
function render(){applyText();const d=state.data,s=d.settings||{},events=d.events||[],posts=d.posts||[],plans=d.plans||[];$("#brandName").textContent=s.brand_name||"Singles Club";document.title=s.page_title||s.brand_name||"Singles Club";renderMap(events);renderPosts(posts);renderPlans(plans);renderFeatured(events);renderOffer(events,plans);renderGoals();renderSchema(s,events)}
function renderMap(events){const map=$("#map");map.querySelectorAll(".pin").forEach(x=>x.remove());const c=[[28,72],[48,82],[69,34],[75,67],[45,48]];events.slice(0,5).forEach((e,i)=>{const b=document.createElement("button");b.className="pin";b.style.left=c[i][1]+"%";b.style.top=c[i][0]+"%";const spots=Math.max(0,Number(e.capacity||0)-Number(e.confirmed_count||0));b.innerHTML=`<b></b><span><strong>${e.city||""}</strong><br>${loc(e,"title")} · ${spots} spots</span>`;b.onclick=()=>choose(`event:${e.id}`);map.appendChild(b)})}
...
function renderFeatured(events){const e=events[0];if(!e){$("#featuredTitle").textContent="—";$("#featuredMeta").textContent="—";return}$("#featuredTitle").textContent=loc(e,"title");$("#featuredMeta").textContent=`${new Date(e.start_at).toLocaleString(state.lang==="zh"?"zh-CN":"en-US",{dateStyle:"medium",timeStyle:"short"})} · ${e.city||""} · $${Number(e.price||0).toFixed(0)}`;$("#featuredBtn").onclick=()=>choose(`event:${e.id}`)}
...
["#openApplySide","#openApplyTop","#openApplyMobile"].forEach(s=>$(s).onclick=openApply);
["#openStatusSide","#openStatusTop","#openStatusMobile"].forEach(s=>$(s).onclick=openStatus);

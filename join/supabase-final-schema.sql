-- LivingHub Join v1.1.1 FINAL PRODUCT SCHEMA
-- Run once in Supabase SQL Editor.
-- Safe to re-run: uses IF EXISTS / IF NOT EXISTS / CREATE OR REPLACE where appropriate.
...
-- PUBLIC DIRECTORY: active profiles only
...
-- registration is only through secure RPC
...
-- keep public upload for early-stage onboarding
...
begin
  ...
  values (
    v_profile.id,
    encode(extensions.digest(p_manage_key,'sha256'),'hex'),
...
begin
  if not exists(
    ...
    where s.profile_id=p_profile_id
      and s.manage_hash=encode(extensions.digest(p_manage_key,'sha256'),'hex')
  ) then
